...
def plotFig(filename, data, fignr=1):
    ...
    for line in ax1.children:
        if line.type == "graph2d.lineseries":
            ...
            counter += 1
    if hasattr(ax1.properties, "XGrid"):
        plt.grid(ax1.properties.XGrid)

    if hasattr(ax1.properties, "XTick"):
        if hasattr(ax1.properties, "XTickLabelRotation"):
            plt.xticks(
                ax1.properties.XTick,
                ax1.properties.XTickLabel,
                rotation=ax1.properties.XTickLabelRotation,
            )
        else:
    ...
    if hasattr(ax1.properties, "YTick"):
        if hasattr(ax1.properties, "YTickLabelRotation"):
            plt.yticks(
                ax1.properties.YTick,
                ax1.properties.YTickLabel,
                rotation=ax1.properties.YTickLabelRotation,
            )
        else:
...
filename_mat = (
    "../../Desktop/Dropbox/transfer_inormation_prague/data/imput/CFD_rats/CFD_S10.mat"
)
