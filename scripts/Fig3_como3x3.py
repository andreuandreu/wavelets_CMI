...
def filter_data(Z, cut=2.0, filter=3, percentage=0.66):
    ...
    filtered_z = scipy.ndimage.zoom(
        Z, filter
    )  # griddata(Z,  method='nearest')#gaussian_filter(Z, sigma=filter)#

    masked_array = np.ma.masked_where(
        (filtered_z > -cut) & (filtered_z < cut), filtered_z
    )
...
def read_plot_all_milan_compludograms(data_names, fig, axes):
    ...
    signame = {"v1": "Sch-IC", "v2": "Im-IC", "v3": "PP-IC"}
    direction = {"AAph": "Amp - Pha", "phAA": "Pha - Amp"}
...
def read_plot_victor_compludograms_by_Zvalue(name, fig, axes, maxZs):
    ...
    filter = 22
    maxZs = [4.0, 4.0, 4.0, 4.0, 4.0, 4.0]
    for key, x, y, psi, maxZ in zip(
        ...
        if "left" in key:
            ...
            im = plot_victor_como(
                axes[key], axeXpoints[cutXlow:cutXhigh], axeYpoints, cutedZ
            )
...
def read_plot_all_victor_compludograms(name, fig, axes, maxZs):
    ...
    for key, n, maxZ in zip(axes, data, maxZs):
        ...
        Zscore = Z_scoring(dataZ)
        filteredZ = filter_data(
            Zscore, cut=2.0, filter=filter
        )  # ,np.ma.masked_where(Zscore < 1.0, Zscore)
        # maxZ = np.max(dataZ)
        # dataZ[0] = maxZ
...
def plot_victor_como(ax, dataX, dataY, dataZ):
    ...
    im = ax.contourf(dataX, dataY, dataZ, extend="both", cmap=cmap, levels=4)  # ,
...
def plot_milan_xyz(ax, dataX, dataY, dataZ):
    ...
    im = ax.contourf(
        axeXpoints, axeYpoints, filteredZ.T, extend="both", cmap=cmap
    )  # contourlevels,
